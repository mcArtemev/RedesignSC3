                ...
                <nav>   <!-- работаю тут -->
                 ...
                 <?php                  }
                    ...
                    <ul class="menu">
                       ...
                       ?>
                       <!-- help-pc DropDownMenu -->
                       <?
                        $showPC=[1,4,5,6]; //добавить это условие в сц7 для того, чтобы url добавлялся именно к сайту с компами
                        //if (isset($this->_datas['admin'])){
                          $site_id = $this->_datas['site_id'];
                            foreach ($all_deall_devices as $key => $value) {
                                      // $setUrl =pdo::getPdo()->query("DELETE FROM `urls` WHERE site_id=$site_id and name = 'help-pc'");
                                      // $setUrl->execute();
                                      // $setUrl = null;
                                if (isset ($value['type_id']) && isset($this->_datas['marka_id'])) {        
                                   if (in_array($value['type_id'],$showPC)) {
                                      
                                      $setParam = serialize(['marka_id' => $this->_datas['marka_id'], 'static' => 'help-pc']);
                                      //include('dbGG.php');
                                      //$this->gg_pdo = new \PDO($sql_dsn_gg, $sql_user_gg, $sql_pass_gg, $sql_opt);
                                      $setUrl =pdo::getPdo(null,'93.95.97.77')->query("INSERT IGNORE INTO `urls` (`site_id`,`name`,`params`,`date`,`feed`) VALUES ($site_id,'help-pc','".$setParam."',".date('U').",".rand(1000, 2100000000).")");
