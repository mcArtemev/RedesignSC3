...
class gen_syns
{
    public function beforeAdd(&$args)
    {
        ...
        $type = (isset($args['type'])) ? ((integer) $args['type']) : -1;
        ...
        foreach ($tables as $table)
        {
            if ($type == -1)
                ...
                $sql = "DELETE FROM `{$table}` WHERE `site_id` IN(".$sites_id_str.") AND `type` = {$type}";              
